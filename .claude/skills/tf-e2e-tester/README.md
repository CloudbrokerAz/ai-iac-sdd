...
This skill validates the complete `/tf-plan-v2` -> `/tf-implement` pipeline after changes to any part of the workflow system — skills, agents, orchestrators, templates, or scripts.

It is **not** a production workflow entry point. It is a testing tool that:

- Runs the full planning workflow (requirements -> design) with auto-resolved decisions via the sdd-design agent
- Runs the full implementation workflow (design -> tests -> code -> sandbox -> deploy -> report) non-interactively via tf-test-writer and tf-task-executor agents
...
- After updating orchestration logic in `/tf-plan-v2` or `/tf-implement`
...
- **`/tf-plan-v2`** — Planning workflow (interactive, user-driven, produces design.md)
- **`/tf-implement`** — Implementation workflow (TDD-aware, reads design.md, writes tests first)
